...
abstract class BaseInterestPeer {
	...
	const QUESTION_ID = 'interestdata.QUESTION_ID';

	
	const USER_ID = 'interestdata.USER_ID';

	
	const CREATED_AT = 'interestdata.CREATED_AT';
	...
	{
		include_once 'lib/model/map/InterestMapBuilder.php';
		return BasePeer::getMapBuilder('lib.model.map.InterestMapBuilder');
	}
	...
	const COUNT = 'COUNT(interestdata.QUESTION_ID)';
	const COUNT_DISTINCT = 'COUNT(DISTINCT interestdata.QUESTION_ID)';
	...
	public static function doCountJoinBlog(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		$criteria->addJoin(InterestPeer::QUESTION_ID, BlogPeer::ID);
	...
	public static function doSelectJoinBlog(Criteria $c, $con = null)
	{
		...
		$c->addJoin(InterestPeer::QUESTION_ID, BlogPeer::ID);
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				$temp_obj2 = $temp_obj1->getBlog(); 				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
										$temp_obj2->addInterest($obj1); 					break;
				}
			}
			if ($newObject) {
				$obj2->initInterests();
		...
		while($rs->next()) {
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				$temp_obj3 = $temp_obj1->getUser(); 				if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey()) {
					$newObject = false;
					$temp_obj3->addInterest($obj1); 					break;
				}
			}
	...
	public static function doDeleteAll($con = null)
	{
	 ...
	 public static function doDelete($values, $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			$criteria = clone $values; 		} elseif ($values instanceof Interest) {
	...
	public static function doValidate(Interest $obj, $cols = null)
	{
		...
		$res =  BasePeer::doValidate(InterestPeer::DATABASE_NAME, InterestPeer::TABLE_NAME, $columns);
    if ($res !== true) {
        $request = sfContext::getInstance()->getRequest();
        foreach ($res as $failed) {
            $col = InterestPeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
...
if (Propel::isInit()) {
		...
		Propel::log('Could not initialize Peer: ' . $e->getMessage(), Propel::LOG_ERR);
			...
			require_once 'lib/model/map/InterestMapBuilder.php';
	Propel::registerMapBuilder('lib.model.map.InterestMapBuilder');
}
